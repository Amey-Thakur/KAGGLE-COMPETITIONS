<div align="center">

# Kaggle Tiers

**The progression ranks and what carries a profile through them.**

<br>

[![Notebooks](https://img.shields.io/badge/Notebooks-Expert-8148FD?logo=kaggle&logoColor=white)](https://www.kaggle.com/ameythakur20/code)
[![Medals](https://img.shields.io/badge/Medals-9_Bronze-8E5B3D)](https://www.kaggle.com/ameythakur20/code)
[![Kaggle](https://img.shields.io/badge/Kaggle-ameythakur20-20BEFF?logo=kaggle&logoColor=white)](https://www.kaggle.com/ameythakur20)

<br>

[Repository home](../../README.md) &nbsp;·&nbsp; [Competitions](../../README.md#competitions) &nbsp;·&nbsp; [Badges](../../Achievements/Badges/README.md) &nbsp;·&nbsp; [Courses](../../Kaggle%20Courses/README.md) &nbsp;·&nbsp; [Kaggle Profile](https://www.kaggle.com/ameythakur20)

</div>

---

Kaggle ranks each category of contribution separately, so a profile carries a
tier for competitions, one for notebooks, one for datasets and one for
discussion. Progression is driven by medals, and the requirements rise steeply
between bands.

<div align="center">

| | | |
| :---: | :---: | :---: |
| <img src="Expert.png" width="72"> | <img src="Master.png" width="72"> | <img src="Grandmaster.png" width="72"> |
| **Expert** | **Master** | **Grandmaster** |

</div>

**Currently Notebooks Expert**, on nine bronze medals, with a highest rank of
875 out of 61,000 in that category.

> [!NOTE]
> Only the highest rank reached is recorded, deliberately. A live position moves
> every time anyone publishes, so a number written into a README is stale the
> day after. The [Kaggle profile](https://www.kaggle.com/ameythakur20) is the
> authority on where the standing sits today.

---

<div align="center">

**Amey Thakur** &nbsp;·&nbsp; [Kaggle](https://www.kaggle.com/ameythakur20) &nbsp;·&nbsp; [GitHub](https://github.com/Amey-Thakur) &nbsp;·&nbsp; [ORCID](https://orcid.org/0000-0001-5644-1575)

<br>

[↑ Back to top](#kaggle-tiers) &nbsp;·&nbsp; [← Repository home](../../README.md)

</div>
